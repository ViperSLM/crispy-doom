...
static patch_t
    *keys[NUMCARDS + 3]; // [crispy] support combined card and skull keys
...
cheatseq_t cheat_powerup[8] = // [crispy] idbehold0
    {
        ...
        CHEAT("idbehold", 0),  CHEAT("idbehold0", 0), // [crispy] idbehold0
};
...
cheatseq_t cheat_massacre4 = CHEAT("kaboom", 0); // ViperSLM
...
void ST_refreshBackground(boolean force)
{
    ...
    if (st_classicstatusbar || force)
    {
        ...
        if ((SCREENWIDTH >> crispy->hires) != ST_WIDTH)
        {
            byte *src;
            pixel_t *dest;
            const char *name = (gamemode == commercial)
                                   ? DEH_String("GRNROCK")
                                   : DEH_String("FLOOR7_2");
            ...
            V_FillFlat(SCREENHEIGHT - (ST_HEIGHT << crispy->hires),
                       SCREENHEIGHT, 0, SCREENWIDTH, src, dest);
            ...
            if (scaledviewwidth == SCREENWIDTH)
            {
                int x;
                patch_t *const patch =
                    W_CacheLumpName(DEH_String("brdr_b"), PU_CACHE);
        ...
        if (!force)
        {
            V_CopyRect(ST_X, 0, st_backing_screen, SCREENWIDTH >> crispy->hires,
                       ST_HEIGHT, ST_X, ST_Y);
        }
        else if (WIDESCREENDELTA > 0 && !st_firsttime)
        {
            V_CopyRect(0, 0, st_backing_screen, WIDESCREENDELTA, ST_HEIGHT, 0,
                       ST_Y);
            V_CopyRect(ORIGWIDTH + WIDESCREENDELTA, 0, st_backing_screen,
                       WIDESCREENDELTA, ST_HEIGHT, ORIGWIDTH + WIDESCREENDELTA,
                       ST_Y);
        }
    }
}
...
static int ST_cheat_spechits()
{
	...
	// ViperSLM: Tag 666 events are stored in it's own function (VSLM_TriggerTag666)
    // dummy.tag = 666;
    ...
    //          (gamemap == 14 || gamemap == 15 || gamemap == 16)))
    //     {
    //         // Mancubi
        ...
        if (!netgame && gameskill != sk_nightmare)
        {
            ...
            if (cht_CheckCheatSP(&cheat_god, ev->data2))
            {
                ...
                if (plyr->playerstate == PST_DEAD)
                {
                    ...
                    mt.angle =
                        (plyr->mo->angle + ANG45 / 2) * (uint64_t) 45 / ANG45;
                    ...
                    P_SpawnMobj(plyr->mo->x + 20 * finecosine[an],
                                plyr->mo->y + 20 * finesine[an], plyr->mo->z,
                                MT_TFOG);
            ...
            else if (cht_CheckCheatSP(&cheat_ammonokey, ev->data2))
            {
                ...
                for (i = 0; i < NUMWEAPONS; i++)
                    if (WeaponAvailable(
                            i)) // [crispy] only give available weapons
            ...
            else if (cht_CheckCheat(&cheat_mus, ev->data2))
            {
                ...
                if (buf[0] == '0' && buf[1] == '0')
                {
                    ...
                    if (gamemode ==
                        commercial /* || gameversion < exe_ultimate */)
                    {
                        musnum =
                          ...
                          : cht_CheckCheat(&cheat_powerup[i], ev->data2))
                {
                    if (!plyr->powers[i])
                        P_GivePower(plyr, i);
                             ...
                             i != pw_allmap) // [crispy] disable full Automap
                        plyr->powers[i] = 1;
                    else
                        plyr->powers[i] = 0;
...
*/
            ...
            // [crispy] implement Boom's "tntem" cheat
                     ...
                     cht_CheckCheatSP(&cheat_massacre4, ev->data2))
            {
                int killcount = ST_cheat_massacre();
                const char *const monster =
                    (gameversion == exe_chex) ? "Flemoid" : "Monster";
                const char *const killed =
                    (gameversion == exe_chex) ? "returned" : "killed";
            ...
            {
				plyr->message = "Triggering Tag 666";
            ...
            else if (cht_CheckCheatSP(&cheat_weapon, ev->data2))
            {
                ...
                else
                {
                    ...
                    {
                        extern boolean P_GiveWeapon(player_t * player,
                                                    weapontype_t weapon,
                                                    boolean dropped);
                        extern const char
                            *const WeaponPickupMessages[NUMWEAPONS];
        ...
        else if (cht_CheckCheat(&cheat_hom, ev->data2))
        {
            ...
            M_snprintf(msg, sizeof(msg), "HOM Detection %s%s", crstr[CR_GREEN],
...
#endif
            M_snprintf(msg, sizeof(msg), "%s (%s) x%ld SDL%s", PACKAGE_STRING,
                       BUILD_DATE, (long) sizeof(void *) * CHAR_BIT,
                       crispy->sdlversion);
#undef BUILD_DATE
            plyr->message = msg;
            fprintf(stderr, "%s\n", msg);
        ...
        // [crispy] eat up the first digit typed after a cheat expecting two parameters
        else if (!netgame && cht_CheckCheat(&cheat_clev1, ev->data2) &&
                 !menuactive)
        {
            char buf[2];
...
#endif
            {
                keyboxes[i] = (plyr->tryopen[i] & KEYBLINKMASK)
                                  ? i + st_keyorskull[i]
                                  : -1;
            }
...
static void ST_doPaletteStuff(void)
{
    ...
    else if (cnt)
    {
        ...
        plyr->bonuscount &&
        plyr->health >
            0) // [crispy] never show the yellow bonus palette for a dead player
    {
...
static inline boolean ST_PlayerIsGibbed(void)
{
    ...
    return (
        plyr->health <= 0 &&
        ((state >= S_PLAY_XDIE1 && state <= S_PLAY_XDIE9) || state == S_GIBS));
}
...
void ST_drawWidgets(boolean refresh)
{
    ...
    if (st_crispyhud)
    {
        ...
        if (plyr->readyweapon == wp_fist && plyr->powers[pw_strength])
        {
            ...
            V_DrawPatch(ST_AMMOX - 21 - SHORT(patch->width) / 2 +
                            SHORT(patch->leftoffset),
                        179 - SHORT(patch->height) / 2 +
...
void ST_Drawer(boolean fullscreen, boolean refresh)
{
    ...
    st_statusbaron =
        (!fullscreen) || (automapactive && !crispy->automapoverlay);
    ...
    st_statusbarface =
        st_classicstatusbar || (st_crispyhud && (screenblocks % 3 == 0));
...
void ST_loadData(void)
{
    ...
    for (i = NUMCARDS; i < NUMCARDS + 3; i++)
    {
        ...
        keys[i] =
            (lumpnum != -1) ? W_CacheLumpNum(lumpnum, PU_STATIC) : keys[i - 3];
    }
}
...
void ST_initData(void)
{
    ...
    faceindex =
        0; // [crispy] fix status bar face hysteresis across level changes
    st_faceindex = 0;
    st_palette = -1;
...
void ST_createWidgets(void)
{
    ...
    STlib_initBinIcon(&w_armsbg, ST_ARMSBGX, ST_ARMSBGY, armsbg,
                      &st_notdeathmatch, &st_classicstatusbar);
...
void ST_Init(void)
{
    ...
    if (!DEH_HasStringReplacement(STSTR_BEHOLD) && !M_ParmExists("-nodeh"))
    {
        ...
        M_snprintf(str_behold, sizeof(str_behold),
                   "in%sV%suln, %sS%str, %sI%snviso, %sR%sad, %sA%sllmap, or "
                   "%sL%site-amp",
                   crstr[CR_GOLD], crstr[CR_NONE], crstr[CR_GOLD],
                   crstr[CR_NONE], crstr[CR_GOLD], crstr[CR_NONE],
                   crstr[CR_GOLD], crstr[CR_NONE], crstr[CR_GOLD],
                   crstr[CR_NONE], crstr[CR_GOLD], crstr[CR_NONE]);
